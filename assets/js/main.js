...
document.addEventListener('DOMContentLoaded', function() {
	...
	let currentImageIndex = 0; // Keeps track of the current image being shown

	// Function to show an image in the lightbox
	function showImage(index) {
		if (index >= 0 && index < images.length) {
			const image = images[index];
			lightboxContent.src = image.querySelector('img').src;
			caption.textContent = image.querySelector('img').alt;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.showImage = function(index) {
        if (index >= 0 && index < images.length) {
            ...
            currentImageIndex = index;
            lightbox.style.display = "flex";
            // Scroll to the work section to ensure the lightbox is fully visible
    ...
    close.onclick = (event) => {
        event.stopPropagation();
        lightbox.style.display = "none";
        // Prevent the template's default behavior which might scroll or navigate
        event.preventDefault();
    }
    ...
    // Handle potential template navigation
    document.querySelectorAll('nav a').forEach(link => {
        link.addEventListener('click', function(event) {
            if (lightbox.style.display === "flex") {
                event.preventDefault(); // Prevent navigation if lightbox is open
